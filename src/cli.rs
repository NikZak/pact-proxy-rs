...
fn unwrap_commandline_args(matches: &clap::ArgMatches) -> Result<(PathBuf, String), Box<dyn Error>> {
    let pact_file = matches.get_one::<PathBuf>("pact_files_folder").ok_or("Can't open pact_file")?.to_path_buf();
...
fn parse_port(matches: &clap::ArgMatches) -> Result<String, Box<dyn Error>> {
    ...
    let port = matches.get_one::<String>("port").unwrap_or(&default_port).to_string();
    Ok(port)
}
