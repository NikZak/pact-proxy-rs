...
impl Pacts {
    pub fn new(pacts: Arc<Mutex<HashMap<(String, String), V4Pact>>>, interaction_index_map: Arc<Mutex<InteractionIndexMap>>, pacts_folder: PathBuf) -> Pacts {
        Pacts {
            pacts,
    ...
    pub fn add_interaction(&self, consumer: &str, provider: &str, request: &HttpRequest, response: &HttpResponse) -> Result<(), Box<dyn Error>> {
        ...
        interaction_index_map.entry((consumer.to_string(), provider.to_string())).or_insert(
            HashMap::new()
        );
    ...
    #[cfg_attr(feature = "flame_it", flame)]
    pub fn get_pact_response(&self, consumer: &str, provider: &str, interaction_descr: &str) -> Option<HttpResponse> {
        ...
        let interaction_index = interaction_index_map.get(&(consumer.to_string(), provider.to_string()))?.get(interaction_descr)?;
    ...
    pub fn save_pact(&self, consumer: &str, provider: &str) -> Result<(), Box<dyn Error>> {
        let pacts = self.pacts.lock().unwrap();
        let pact = pacts.get(&(consumer.to_string(), provider.to_string())).unwrap();
